...
// (due to length launched as separate kernel before other 
// backward steps contained in preprocess)
__global__ void computePanoramaCov2DCUDA(int P,
	const float3* means,
	const int* radii,
	const float* cov3Ds,
	...
	glm::vec4* dL_drot)
{
	...
	if (width == 2 * height)
	{
		...
		float dL_dtx = dL_dpx * dpx_dtx + dL_dpy * dpy_dtx;
		float dL_dty = dL_dpy * dpy_dty;
		float dL_dtz = dL_dpx * dpx_dtz + dL_dpy * dpy_dtz;
		float3 dL_dmean3D = transformVec4x3Transpose({ dL_dtx, dL_dty, dL_dtz }, view);
		glm::vec3 dL_dmean(dL_dmean3D.x, dL_dmean3D.y, dL_dmean3D.z);
		// That's the second part of the mean gradient. Previous computation
		// of cov2D and following SH conversion also affects it.
		dL_dmeans[idx] += dL_dmean;

		float x2y2z2_sqrt_inv = 1.0f / sqrtf(m.x * m.x + m.y * m.y + m.z * m.z + 0.0000001f);
		// Compute loss gradient w.r.t. 3D means due to gradients of depth
		// from rendering procedure
		dL_dmean2.x = (m.x * x2y2z2_sqrt_inv) * dL_ddepth[idx];
		dL_dmean2.y = (m.y * x2y2z2_sqrt_inv) * dL_ddepth[idx];
		dL_dmean2.z = (m.z * x2y2z2_sqrt_inv) * dL_ddepth[idx];
...
renderCUDA(
	...
	const float* __restrict__ dL_dconfidencepixels,
	...
	float* __restrict__ dL_ddepths) 
{
	...
	__shared__ float collected_feats[NUM_FEATURE_CHANNELS * BLOCK_SIZE];
	__shared__ float collected_conf[BLOCK_SIZE];
	...
	float dL_dconfidencepixel[1];
	if (confidence && inside)
		dL_dconfidencepixel[0] = dL_dconfidencepixels[pix_id];

	float last_alpha = 0;
	float last_color[C] = { 0 };
	float last_feat[NUM_FEATURE_CHANNELS] = { 0 };
	float last_conf = 0;
	float last_depth = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < NUM_FEATURE_CHANNELS; i++)
				collected_feats[i * BLOCK_SIZE + block.thread_rank()] = features[coll_id * NUM_FEATURE_CHANNELS + i];			
			collected_conf[block.thread_rank()] = confidence[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (features)
				for (int ch = 0; ch < NUM_FEATURE_CHANNELS; ch++) { 
					...
					const float f = collected_feats[ch * BLOCK_SIZE + j];
					...
					// Update the gradients w.r.t. features of the Gaussian.
					atomicAdd(&(dL_dfeatures[global_id * NUM_FEATURE_CHANNELS + ch]), dchannel_dcolor * dL_dfeaturepixel[ch]); 			
				}		
			
			if (confidence) {

				const float conf = collected_conf[j];
				accum_conf_rec = last_alpha * last_conf + (1.f - last_alpha) * accum_conf_rec;
				last_conf = conf;

				dL_dopa += (conf - accum_conf_rec) * dL_dconfidencepixel[0];

				atomicAdd(&(dL_dconfidence[global_id]), dchannel_dcolor * dL_dconfidencepixel[0]);
			}
	...
	glm::vec4* dL_drot)
{
	...
	if (W == 2 * H)
	{
		computePanoramaCov2DCUDA << <(P + 255) / 256, 256 >> > (
			P,
			means3D,
			radii,
			cov3Ds,
...
void BACKWARD::render(
	...
	const float* dL_dconfidencepixels,
