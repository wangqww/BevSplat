...
class GSDownSample(nn.Module):
    def __init__(self, level, estimate_depth=0):
        super(GSDownSample, self).__init__()
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 若 in_channels != out_channels，使用1x1卷积调整通道数
        self.skip_conv = None
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_resblock=True):
        super(UpBlock, self).__init__()
        # 使用双线性上采样，也可换成转置卷积等方式
...
class GSUpSample(nn.Module):
    def __init__(self):
        super(GSUpSample, self).__init__()
        # 假定输入是 (1,512,64,64)
        # 1) 降维但不改变空间大小: 512 -> 256
        self.down_conv = nn.Conv2d(512, 256, kernel_size=1)
        self.down_bn = nn.BatchNorm2d(256)
        self.down_relu = nn.ReLU(inplace=True)

        # 2) 上采样到 (1,128,128,128)
        self.up1 = UpBlock(in_channels=256, out_channels=128, use_resblock=True)

        # 3) 再上采样到 (1,64,256,256)
        self.up2 = UpBlock(in_channels=128, out_channels=64, use_resblock=True)

        # 如果有跳跃连接，可以在此定义。例如：
        # self.skip_conv = nn.Conv2d(其他特征通道,对应的输出通道, kernel_size=1)
        # 在 forward 中进行拼接 (如 x = torch.cat([x, skip_feat], dim=1)) 再卷积。

    def forward(self, x):
        # x: (1,512,64,64)
        x1 = self.down_conv(x)  # (1,256,64,64)
        x2 = self.down_bn(x1)
        x2 = self.down_relu(x2)

        x2 = self.up1(x2)  # (1,128,128,128)
        x3 = self.up2(x2)  # (1,64,256,256)
