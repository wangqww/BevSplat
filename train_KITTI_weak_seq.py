...
def test1(net_test, args, save_path, epoch):
    ...
    with torch.no_grad():
        for i, Data in enumerate(dataloader, 0):
            sat_map, left_camera_k, grd_left_imgs, grd_left_imgs_ori, grd_depth, gt_shift_u, gt_shift_v, gt_heading, loc_shift_left, heading_shift_left, real_gps = [item.to(device) for item in Data[:-1]]

            sat_feat_dict, sat_conf_dict, g2s_feat_dict, g2s_conf_dict, mask_dict, shift_lats, shift_lons, thetas, render_loss = \
                net(sat_map, grd_left_imgs, grd_left_imgs_ori, grd_depth, left_camera_k, gt_shift_u, gt_shift_v, gt_heading, loc_shift_left, heading_shift_left)
    ...
    for idx in range(len(metrics)):
        ...
        line = 'distance within ' + str(metrics[idx]) + ' meters (init, pred by corr' + str(init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(metrics)):
        ...
        line = 'lateral within ' + str(metrics[idx]) + ' meters (init, pred by corr' + str(init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(metrics)):
        ...
        line = 'longitudinal within ' + str(metrics[idx]) + ' meters (init, pred by corr' + str(init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(angles)):
        pred = np.sum(angle_diff < angles[idx]) / angle_diff.shape[0] * 100
        init = np.sum(init_angle < angles[idx]) / angle_diff.shape[0] * 100
        line = 'angle within ' + str(angles[idx]) + ' degrees (init, pred by corr): ' + str(init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
...
def test2(net_test, args, save_path, epoch):
    ...
    line = 'Distance average: (init, pred by corr)' + str(np.mean(init_dis)) + ' ' + str(np.mean(distance))
    print(line)
    f.write(line + '\n')
    line = 'Distance median: (init, pred by corr)' + str(np.median(init_dis)) + ' ' + str(np.median(distance))
    print(line)
    f.write(line + '\n')

    line = 'Lateral average: (init, pred by corr)' + str(np.mean(np.abs(gt_lats))) + ' ' + str(np.mean(diff_lats))
    print(line)
    f.write(line + '\n')
    line = 'Lateral median: (init, pred by corr)' + str(np.median(np.abs(gt_lats))) + ' ' + str(np.median(diff_lats))
    print(line)
    f.write(line + '\n')

    line = 'Longitudinal average: (init, pred by corr)' + str(np.mean(np.abs(gt_lons))) + ' ' + str(np.mean(diff_lons))
    print(line)
    f.write(line + '\n')
    line = 'Longitudinal median: (init, pred by corr)' + str(np.median(np.abs(gt_lons))) + ' ' + str(np.median(diff_lons))
    print(line)
    f.write(line + '\n')
    ...
    for idx in range(len(metrics)):
        ...
        line = 'distance within ' + str(metrics[idx]) + ' meters (init, pred by corr): ' + str(
            init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(metrics)):
        ...
        line = 'lateral within ' + str(metrics[idx]) + ' meters (init, pred by corr): ' + str(
            init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(metrics)):
        ...
        line = 'longitudinal within ' + str(metrics[idx]) + ' meters (init, pred by corr): ' + str(
            init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
    ...
    for idx in range(len(angles)):
        pred = np.sum(angle_diff < angles[idx]) / angle_diff.shape[0] * 100
        init = np.sum(init_angle < angles[idx]) / angle_diff.shape[0] * 100
        line = 'angle within ' + str(angles[idx]) + ' degrees (init, pred by corr): ' + str(
            init) + ' ' + str(pred)
        print(line)
        f.write(line + '\n')
...
def train(net, args, save_path, name_path):
    ...
    if args.share:
        ...
        params = list(net.feat_gaussian_encoder.parameters()) + list(net.dpt.parameters()) + list(net.FuseNet_feat.parameters()) + list(net.FuseNet_conf.parameters())
    else:
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        path = 'ModelsKitti/3DoF/Stage4/lat20.0m_lon20.0m_rot0.0_Nit1_TransV1_geo_Level1_Channels32_16_4_Share_feat32_offset_0.5_seq4_1e-4/model_8.pth'
